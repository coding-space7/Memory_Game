...
SQUARE_SIZE = 50
SQUARE_GAP = 10
BOARD_WIDTH = 8  # no. of rows
BOARD_HEIGHT = 4  # no.of columns
...
assert (BOARD_HEIGHT * BOARD_WIDTH) % 2 == 0, 'The board size must be even'  # if the condition is False then program
# terminates
...
def get_pos(cx, cy):
    ...
    if x >= BOARD_HEIGHT or y >= BOARD_WIDTH or (cx - X_MARGIN) % (SQUARE_SIZE + SQUARE_GAP) > SQUARE_SIZE or (
            cy - Y_MARGIN) % (SQUARE_SIZE + SQUARE_GAP) > SQUARE_SIZE:
        return None, None
    else:
        return x, y
...
def draw_square(board, revealed, x, y):
    ...
    pg.draw.rect(screen, BACKGROUND_COLOR, square_rect)
    if revealed[x][y]:
